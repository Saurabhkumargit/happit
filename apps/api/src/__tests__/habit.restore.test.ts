...
async function createAuthenticatedAgent() {
  ...
  const email = `habit-restore-${crypto.randomUUID()}@example.com`;
...
describe("POST /api/v1/habits/:habitId/restore", () => {
  afterAll(async () => {
  ...
  it("requires authentication", async () => {
    await request(app)
      .post(
        "/api/v1/habits/00000000-0000-0000-0000-000000000000/restore",
      )
      .expect(401);
  });
  ...
  it("restores an archived habit", async () => {
    ...
    const response = await agent
      .post(`/api/v1/habits/${exerciseId}/restore`)
      .expect(200);
  ...
  it("does not allow a user to restore another user's habit", async () => {
  ...
  it("rejects restoring an already active habit", async () => {
    const agent = await createAuthenticatedAgent();
    const exerciseId = await getExerciseId(agent);
