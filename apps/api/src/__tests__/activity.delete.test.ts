...
async function createAuthenticatedAgent() {
  ...
  const email = `activity-delete-${crypto.randomUUID()}@example.com`;
...
describe("DELETE /api/v1/activities/:activityId", () => {
  ...
  it("deletes the authenticated user's activity", async () => {
  ...
  it("does not allow deleting another user's activity", async () => {
  ...
  it("rejects an invalid activity ID", async () => {
    ...
    const response = await agent
      .delete("/api/v1/activities/not-a-uuid")
      .expect(400);
